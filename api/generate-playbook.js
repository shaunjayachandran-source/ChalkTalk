...
- Named position anchors (half-court) -- use these exactly, do not invent your own coordinates for these spots:
  ...
  Dunker spot (baseline finishing spot just OUTSIDE the lane line, below and outside the Block -- between the Block and the baseline; a DIFFERENT spot from Block, never reuse the Block coordinate for it, and never put two players on the same horizontal line here): right cx=326, left cx=194 (cy=357 for DOWN, cy=150 for UP).
  ...
  Inbounder (BLOB out-of-bounds passer -- stands in the out-of-bounds strip just BEHIND the baseline, lined up with where the lane line would extend past the baseline; NEVER directly under or behind the basket itself. The court art's baseline sits at y=370 (DOWN) / y=137 (UP), so this spot is still on the visible court image. Pick whichever side the play's alignment calls for): right cx=313, left cx=207 (cy=382 for DOWN, cy=125 for UP).
  Short corner (between the block and the deep corner, still near the baseline): right cx=388, left cx=132 (cy=358 for DOWN, cy=149 for UP).
