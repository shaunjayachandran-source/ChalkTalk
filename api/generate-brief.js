...
FLEX CUT RULE (critical -- this is a real basketball structure rule, not a style preference): in a flex/flex-continuity action, the baseline cutter ALWAYS starts in the Deep corner on the WEAK side -- the side of the court OPPOSITE wherever the ball is at that moment -- and cuts along the baseline to the Block anchor on the STRONG side (the ball's side). Determine which side is which the same way as the SCREENER POSITIONING RULE above: compare the ball-handler's x position to court-center x=260. If the ball-handler's x is less than 260 (ball on the LEFT / strong side), the flex cutter starts in the RIGHT Deep corner (the weak side) and cuts to the LEFT Block. If the ball-handler's x is 260 or greater (ball on the RIGHT / strong side), the flex cutter starts in the LEFT Deep corner and cuts to the RIGHT Block. NEVER start the flex cutter in the corner on the SAME side as the ball -- a cutter starting ball-side and finishing ball-side is not a flex cut, it is a basketball-incorrect reversal of the real action, even if every individual coordinate looks plausible in isolation. The screener for this action is a DIFFERENT teammate, positioned at the Screen spot outside the block anchor above on the SAME (strong) side as the cutter's destination -- NOT the Block anchor itself (that is the cutter's endpoint, not the screener's standing spot) and NOT an elbow (that is the separate SCREENER POSITIONING RULE above, for a different action). State the weak-side start and strong-side destination explicitly in that phase's action text (e.g. "Cuts baseline from the weak-side [right] corner to the strong-side [left] block off 4's screen").
...
SIDE DETERMINATION MUST USE THE BALL'S POSITION AT THE END OF THIS SAME PHASE, NOT ITS START (critical -- this is what makes the SCREENER POSITIONING RULE and FLEX CUT RULE above work correctly across a ball reversal, and a confirmed real bug when skipped): a ball reversal (a dribble or pass that moves the ball from one side to the other) and a teammate's early positioning move toward an upcoming screen spot often happen in the SAME phase -- e.g. phase N has "1 dribbles from the right slot to the left slot" AND, in that same phase, "5 relocates from the right corner toward a screen spot in preparation for the next phase." In this situation, the screener/preparing player's destination side MUST be computed from the ball-handler's ENDING x position for that same phase (i.e. after the dribble/pass completes), never their STARTING x position -- the preparing player is anticipating where the ball is going to be, not reacting to where it already was. Getting this backwards (using the ball's pre-reversal side) strands the screener on the wrong side for the whole rest of the action: the flex cutter can still correctly start on the weak side relative to the ball's new position, but if the screener/preparation move used the OLD ball side, the cutter's path and the screener's screen never actually meet on the same side of the court, breaking the entire action even though each individual player's own rule was technically followed in isolation. Before finalizing any phase where a player is moving toward a screen spot for a LATER phase's action, re-check: does the ball also move THIS phase? If yes, use where the ball ENDS UP this phase, not where it started.
